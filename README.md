...
| **8** | Skip stack. Each 8 queues one skip. While the queue is alive, only another 8 plays. Also cancels a pickup chain — playing an 8 on a 2 (or any +N chain) clears the chain and queues a skip on the next player. **Cannot follow a Jack.** |
...
| **J** | Plays on another Jack or in suit. Black Jacks (♠ ♣) add +5 to the pickup chain; red Jacks (♥ ♦) cancel a black Jack. **While the deck still has cards, a red Jack can ONLY be played to cancel a pending black Jack** — without a BJ to cancel, red J is blocked. Once the deck is empty, red Js fall back to normal Jack-card rules. **Blocks 8** as the lead on a Jack-topped pile. **Blocks Ace** specifically on a black Jack while the deck still has cards (Ace plays on a red Jack always, and on a black Jack once the deck is empty). |
