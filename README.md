...
| **7** | Lower-or-equal lock. Plays per the normal rule (not wild). When a 7 is on top, the next player must play rank ≤ 7 or a 10. **In your own hand chain, a same-suit 8 may chain after a 7** (e.g., 7♠ → 8♠) as a run extension; all other ranks > 7 stay blocked. |
...
| **A** | Wild. Blocked on a 2 only while the pickup chain is alive — once the chain is empty (taken, 10-burned, or never formed because the deck was empty when the 2 was played), Ace plays on the 2 normally. Blocked on a freshly-placed black Jack while the deck still has cards (stale BJ unblocks; deck-empty BJ also unblocks). Always blocked after an 8 (skip-lock). Plays on a red Jack always. Names the suit the next player must follow. **In a hand chain, Ace and 2 chain together regardless of suit** (2 → A or A → 2) — they pair as wraparound neighbours in either direction. |
