...
| **A** | Wild. **Cannot be played anytime the pickup chain is alive** (universal block). With chain empty: blocked on a freshly-placed black Jack while the deck still has cards (stale BJ unblocks; deck-empty BJ also unblocks). Cannot be played on a 7 (7-lock blocks all > 7). Plays on an 8 once the skip queue has drained. Plays on a red Jack always. Names the suit the next player must follow. **In a hand chain: 2 → A pairs regardless of suit; A → 2 only chains when the 2's suit matches the Ace's called suit.** |
